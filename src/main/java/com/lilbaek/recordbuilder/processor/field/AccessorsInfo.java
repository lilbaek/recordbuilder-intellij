package com.lilbaek.recordbuilder.processor.field;

import com.intellij.psi.PsiAnnotation;
import com.intellij.psi.PsiClass;
import com.intellij.psi.PsiField;
import com.intellij.psi.PsiVariable;
import com.lilbaek.recordbuilder.util.PsiAnnotationUtil;
...
public class AccessorsInfo {
    ...
    private static AccessorsInfo buildAccessorsInfo(@Nullable PsiClass psiClass, @Nullable Boolean chainDeclaredValue,
        ...
        String[] prefixes = prefixDeclared.toArray(new String[0]);

        // TODO: Cleanup
        if (null != psiClass) {
            // final ConfigDiscovery configDiscovery = ConfigDiscovery.getInstance();
            if (null == fluentDeclaredValue) {
                //isFluent = configDiscovery.getBooleanRConfigProperty(ConfigKey.ACCESSORS_FLUENT, psiClass);
            } else {
                isFluent = fluentDeclaredValue;
            }

            if (null == chainDeclaredValue) {
                //isChained = configDiscovery.getBooleanRConfigProperty(ConfigKey.ACCESSORS_CHAIN, psiClass);
            } else {
                isChained = chainDeclaredValue;
            }

            if (prefixDeclared.isEmpty()) {
                //prefixes = configDiscovery.getMultipleValueRConfigProperty(ConfigKey.ACCESSORS_PREFIX, psiClass);
            } else {
                prefixes = prefixDeclared.toArray(new String[0]);
            }

            //doNotUseIsPrefix = configDiscovery.getBooleanRConfigProperty(ConfigKey.GETTER_NO_IS_PREFIX, psiClass);
    ...
    public String removePrefix(String fieldName) {
        for (String prefix : prefixes) {
            if (canPrefixApply(fieldName, prefix)) {
                return prefix.isEmpty() ? fieldName : decapitalizeLike(fieldName.substring(prefix.length()));
            }
        }
        return fieldName;
    }
    ...
    private boolean canPrefixApply(String fieldName, String prefix) {
        ...
        return prefixLength == 0 ||
                        fieldName.startsWith(prefix) && fieldName.length() > prefixLength &&
                                        (!Character.isLetter(prefix.charAt(prefix.length() - 1)) || !Character.isLowerCase(
                                                        fieldName.charAt(prefixLength)));
    }

    private String decapitalizeLike(String name) {
